...
// Live endpoint — 10-min server-side cache, triggers Google Maps refresh when stale.
// Returns { destinations, last_updated, cache_age_minutes }
export async function getLiveDestinations(origin = null) {
  const url = origin
    ? `${API_BASE}/live-times?origin=${encodeURIComponent(origin)}`
    : `${API_BASE}/live-times`;
  const res = await fetch(url, { cache: 'no-store' }); // always bypass Next.js/CDN cache
  if (!res.ok) throw new Error('Failed to fetch live times');
  return res.json(); // { destinations, last_updated, cache_age_minutes }
}
